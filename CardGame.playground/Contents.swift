...
extension CardRanks: CustomStringConvertible, CaseIterable, Comparable {
...
enum Suits: String, CaseIterable, Comparable {
    static func < (lhs: Suits, rhs: Suits) -> Bool {
        return lhs.rawValue < rhs.rawValue
    }
    ...
    init() {
        ...
        for rank in CardRanks.allCases {
            for suit in Suits.allCases {
                dealerCards.append(Card(suit: suit, rank: rank))
            }
        }
        
        self.cards = dealerCards
        
    }
    
    func draw() -> Card {
      
        let randomNum = Int.random(in: 0...51)
...
protocol CardGameDelegate {
    ...
    func cardGamedidstate(_ game: CardGame)
...
class HighLow: CardGame {
    ...
    func play() {
        let player1 = deck.draw()
        let player2 = deck.draw()
        
        
        if player1.rank == player2.rank {
            print("This High Low game has ended in a tie!")
...
class CardGameTracker: CardGameDelegate {
    ...
    func cardGamedidstate(_ game: CardGame) {
        ...
        if game is HighLow {
            ...
            print("The card game HighLow has begun!")
        }
    }
    
    func game(player1DidDraw card1: Card, player2DidDraw card2: Card) {
        numberOfTurns += 1
        print("player 1 hand: \(card1.description),  player 2 hand: \(card2.description)")
    }
...
//: ```
let cardGameDelegate = CardGameTracker()
let highLowGame = HighLow(deck: Deck(), delegate: cardGameDelegate)
